package com.xebia.kafka.connect.couchdb.example;
...
public class JsonProducerExample {
  ...
  private static final String TOPIC = "couchdb-sink-example";
  ...
  private static Producer<String, byte[]> createProducer() {
    ...
    config.put(ProducerConfig.CLIENT_ID_CONFIG, "CouchDBJsonProducerExample");
  ...
  private static void publishMessage(Producer<String, byte[]> producer) throws Exception {
    // Try setting the key to null and see how the CouchDB Sink Connector behaves.
    // Try to see how the CouchDB Sink Connector behaves when configuring the CouchDB Sink Connector with the
    // property:
    //     topics-to-id-fields-mapping=couchdb-sink-example/airport
  ...
  private static ObjectNode randomWeatherReport() {
    ...
    report.put("id", random.nextInt(airports.size()) + "");
